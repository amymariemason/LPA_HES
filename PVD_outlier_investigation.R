...
TLall<-merge(TLframe, traitnames, by="trait", all.x=TRUE)
  ...
  # print outcome name
  outcomespec<-as.character(TLall[4,"trait"])
  outcomename<-as.character(TLall[4,"name"])
  ...
  outcome3<-outcome3[outcome3$pos!="160543317",]
  ...
  outlier<-summary_temp

#save and add to report
save(outlier, file = "./Outputs/PAD_outlier.RData")

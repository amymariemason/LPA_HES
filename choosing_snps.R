...
keep2<- merge(keep, lpa_label, by.y=c("pos", "allele1", "allele2"), by.x=c("position","alleleA","alleleB"), all.x=TRUE)
keep2<-keep2[!is.na(keep2$variantID),]
keep3<- merge(keep, lpa_label, by.y=c("pos", "allele1", "allele2"), by.x=c("position","alleleB","alleleA"), all.x=TRUE)
keep3<-keep3[!is.na(keep3$variantID),]
keep<-rbind(keep2,keep3)
rm(keep2, keep3)

write.table(keep,file="//me-filer1/home$/am2609/My Documents/Programs/GWAS_inprogress/Outputs/LPA_vars.txt", row.names = FALSE)

##################################################################
# Determine best LP(A) snps SCORE with snps present in UKBB 
# Author: Amy Mason
# Date: Jan 2018
##################################################################
#################################################################
...
#########################################################################
# Look up snps that are in 935 lp(a) list and UK biobank
#########################################################################
...
# subset UKBB data to that which overlaps with lpa

try<-import_data(statsfile, inputname="position", lpa, varname="pos") #to subset by UKBB
keep<-as.data.frame(try[[1]])
message(nrow(keep), " variants in UKBB and lpa var list")

# check for duplicates by position/ label with variant ID

lpa_label<-lpa[,c("variantID", "chr", "pos", "allele1", "allele2")]
keepA<- merge(keep, lpa_label, by.y=c("pos", "allele1", "allele2"), by.x=c("position","alleleA","alleleB"), all.x=TRUE)
keepA<-keepA[!is.na(keepA$variantID),]
keepB<- merge(keep, lpa_label, by.y=c("pos", "allele1", "allele2"), by.x=c("position","alleleB","alleleA"), all.x=TRUE)
keepB<-keepB[!is.na(keepB$variantID),]
keep3<-rbind(keepA,keepB)
rm(keepA, keepB)

write.table(keep3,file="//me-filer1/home$/am2609/My Documents/Programs/GWAS_inprogress/Outputs/LPA_UKBB_vars.txt", row.names = FALSE)
rm(keep, keep3)
...
Vars<-read.table("//me-filer1/home$/am2609/My Documents/Programs/GWAS_inprogress/Outputs/LPA_UKBB_vars.txt", header=T)
...
write.table(Vars[Vars$QCfail==0,],file="//me-filer1/home$/am2609/My Documents/Programs/GWAS_inprogress/Outputs/QCed_LPA&BB&C4D_vars.txt", row.names = FALSE)
...
write.table(results, "//me-filer1/home$/am2609/My Documents/Programs/GWAS_inprogress/Outputs/fstep_UKBB_LPA_snps.txt", row.names=F, quote=F, sep="\t")
